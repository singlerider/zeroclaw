...
use crate::scattered_types::{AutonomyLevel, DomainMatcher, SopConfig};

// Provider alias helpers (inlined to avoid circular dep with providers crate)
fn is_glm_global_alias(name: &str) -> bool {
    matches!(name, "glm" | "zhipu" | "glm-global" | "zhipu-global")
}
fn is_glm_cn_alias(name: &str) -> bool {
    matches!(name, "glm-cn" | "zhipu-cn")
}
fn is_glm_alias(name: &str) -> bool {
    is_glm_global_alias(name) || is_glm_cn_alias(name)
}
fn is_zai_global_alias(name: &str) -> bool {
    matches!(name, "zai" | "z.ai" | "zai-global" | "z.ai-global")
}
fn is_zai_cn_alias(name: &str) -> bool {
    matches!(name, "zai-cn" | "z.ai-cn")
}
fn is_zai_alias(name: &str) -> bool {
...
pub struct Config {
    ...
    #[serde(default)]
    pub browser_delegate: crate::BrowserDelegateConfig,
...
pub struct AgentConfig {
    ...
    #[serde(default)]
    pub history_pruning: crate::HistoryPrunerConfig,
    ...
    #[serde(default)]
    pub eval: crate::EvalConfig,

    /// Automatic complexity-based classification fallback.
    #[serde(default)]
    pub auto_classify: Option<crate::AutoClassifyConfig>,

    /// Context compression configuration for automatic conversation compaction.
    #[serde(default)]
    pub context_compression: crate::ContextCompressionConfig,
...
impl Default for AgentConfig {
    fn default() -> Self {
        Self {
            ...
            history_pruning: crate::HistoryPrunerConfig::default(),
            context_aware_tools: false,
            eval: crate::EvalConfig::default(),
            auto_classify: None,
            context_compression:
                crate::ContextCompressionConfig::default(),
...
pub struct ChannelsConfig {
    ...
    pub clawdtalk: Option<crate::ClawdTalkConfig>,
    ...
    pub voice_call: Option<crate::VoiceCallConfig>,
...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            ...
            trust: crate::TrustConfig::default(),
            ...
            browser_delegate: crate::BrowserDelegateConfig::default(),
...
fn decrypt_optional_secret(
    ...
    if let Some(raw) = value.clone() {
        if crate::SecretStore::is_encrypted(&raw) {
            *value = Some(
                store
                    .decrypt(&raw)
...
fn decrypt_secret(
    ...
    if crate::SecretStore::is_encrypted(value) {
        *value = store
            .decrypt(value)
            .with_context(|| format!("Failed to decrypt {field_name}"))?;
    }
...
impl Config {
    pub async fn load_or_init() -> Result<Self> {
        ...
        if config_path.exists() {
            ...
            let store = crate::SecretStore::new(&zeroclaw_dir, config.secrets.encrypt);
    ...
    pub async fn save(&self) -> Result<()> {
        ...
        let store = crate::SecretStore::new(zeroclaw_dir, self.secrets.encrypt);
...
"#;
    ...
    async fn config_save_and_load_tmpdir() {
        ...
        assert!(
            loaded
                .api_key
                .as_deref()
                .is_some_and(crate::SecretStore::is_encrypted)
        );
        let store = crate::SecretStore::new(&dir, true);
    ...
    async fn config_save_encrypts_nested_credentials() {
        ...
        assert!(crate::SecretStore::is_encrypted(root_encrypted));
        ...
        assert!(crate::SecretStore::is_encrypted(
            composio_encrypted
        ));
        assert_eq!(
        ...
        assert!(crate::SecretStore::is_encrypted(worker_encrypted));
        ...
        assert!(crate::SecretStore::is_encrypted(storage_db_url));
        assert_eq!(
...
"#,
    ...
    async fn channel_secret_telegram_bot_token_roundtrip() {
        ...
        assert!(
            crate::SecretStore::is_encrypted(stored_token),
        ...
        let load_store = crate::SecretStore::new(&dir, loaded.secrets.encrypt);
    ...
    async fn nevis_client_secret_encrypt_decrypt_roundtrip() {
        ...
        assert!(
            crate::SecretStore::is_encrypted(stored_secret),

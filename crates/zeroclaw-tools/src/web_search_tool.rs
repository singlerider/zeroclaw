use zeroclaw_types::tool::{Tool, ToolResult};
...
impl WebSearchTool {
    ...
    fn resolve_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if let Some(ref key) = self.boot_brave_api_key {
            if !key.is_empty() && !zeroclaw_config::SecretStore::is_encrypted(key) {
                return Ok(key.clone());
            }
        }
    ...
    fn reload_brave_api_key(&self) -> anyhow::Result<String> {
        ...
        if zeroclaw_config::SecretStore::is_encrypted(&raw_key) {
            let zeroclaw_dir = self.config_path.parent().unwrap_or_else(|| Path::new("."));
            let store = zeroclaw_config::SecretStore::new(zeroclaw_dir, self.secrets_encrypt);
...
mod tests {
    ...
    fn test_resolve_brave_api_key_decrypts_encrypted_key() {
        let tmp = tempfile::TempDir::new().unwrap();
        let store = zeroclaw_config::SecretStore::new(tmp.path(), true);

...
- **Streaming completion follows protocol events.** Providers finish a successful stream when its terminal SSE event arrives rather than waiting for the server to close the connection. Silent response bodies fail after the shared byte-idle timeout, while active generations may outlive the non-streaming request timeout.

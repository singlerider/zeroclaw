...
ZeroClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers via the `custom:` and `anthropic-custom:` provider scheme prefixes — set `default_provider`, `api_key`, and `default_model` accordingly. See the [Config reference](../reference/config.md) for the full field index.

For `custom:` and `anthropic-custom:` providers, the generic key env vars `API_KEY` and `ZEROCLAW_API_KEY` apply.
